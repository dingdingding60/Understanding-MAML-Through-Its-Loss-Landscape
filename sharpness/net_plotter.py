...
def get_random_weights(weights,freeze=False):
    """
        Produce a random direction that is a list of random Gaussian tensors
        with the same shape as the network's weights, so one direction entry per weight.
    """
    if freeze=="conv":   
        a =[torch.randn(w.size()) for w in weights]
        for x in a[:-2]:
            x.zero_()
        return a
    elif freeze=="fc":   
...
def create_random_direction(net, dir_type='weights', ignore='biasbn', norm='filter',freeze="None"):
    """
    ...
    if dir_type == 'weights':
        ...
        direction = get_random_weights(weights,freeze)
...
def setup_direction(args, dir_file, net):
    ...
    if not args.dir_file:
        ...
        if args.model_file2:
            ...
            xdirection = create_random_direction(net, args.dir_type, args.xignore, args.xnorm,args.freeze)
        ...
        if args.y:
            if args.same_dir:
                ...
                ydirection = create_random_direction(net, args.dir_type, args.yignore, args.ynorm,args.freeze)
...
def name_direction_file(args):
    ...
    #             dir_file += '_same_dir'
    dir_file += "_exp"+str(args.idx)+"_"+args.alg[0]+"_"+args.split+"_"+str(args.n_way)+"way_"+str(args.k_shot)+"shot_"+str(args.test_task)+"ttsk_"+str(args.n_query)+"query"
    if args.freeze != "None":
        dir_file+='_freeze'+args.freeze
    # index number
    dir_file += ".h5"
    return dir_file
