...
def plot_2d_contour(args, surf_name='train_loss', show=False):
    ...
    # fig.savefig(args.sf + '_1' + surf_name + '_2dcontour' + '.pdf', dpi=300,
    ...
    fig.savefig(args.sf + '_scale' + surf_name + '_3dsurface.pdf', dpi=300,
                bbox_inches='tight', format='pdf')
    fig = plt.figure()
    ax = Axes3D(fig)
    surf = ax.plot_surface(X, -Y, Z, cmap=cm.coolwarm, linewidth=0, antialiased=False)
    ax.set_zlim(zmin,zmax)
    fig.colorbar(surf, shrink=0.5, aspect=5)
    fig.savefig(args.sf + '_1_scale' + surf_name + '_3dsurface.pdf', dpi=300,
                bbox_inches='tight', format='pdf')
    fig = plt.figure()
    ax = Axes3D(fig)
    surf = ax.plot_surface(-X, Y, Z, cmap=cm.coolwarm, linewidth=0, antialiased=False)
    ax.set_zlim(zmin,zmax)
    fig.colorbar(surf, shrink=0.5, aspect=5)
    fig.savefig(args.sf + '_2_scale' + surf_name + '_3dsurface.pdf', dpi=300,
                bbox_inches='tight', format='pdf')
    fig = plt.figure()
    ax = Axes3D(fig)
    surf = ax.plot_surface(-X, -Y, Z, cmap=cm.coolwarm, linewidth=0, antialiased=False)
    ax.set_zlim(zmin,zmax)
    # norm = Normalize(vmin=3.8, vmax=10.3)
    fig.colorbar(surf, shrink=0.5, aspect=5)
    fig.savefig(args.sf + '_3_scale' + surf_name + '_3dsurface.pdf', dpi=300,
                bbox_inches='tight', format='pdf')
    f.close()
    if show: plt.show()
