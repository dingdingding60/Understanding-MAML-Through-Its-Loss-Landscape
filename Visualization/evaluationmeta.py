...
from META.data.datamgr import SetDataManager,SetDataManager_task,SimpleDataManager
...
def feature_evaluation(cl_data_file, model, n_way = 5, n_support = 5, n_query = 15, adaptation = False):
    ...
    if adaptation:
        scores,losss,lossq  = model.set_forward_adaptation(z_all, is_feature = True)
    else:
        scores,losss,lossq  = model.set_forward(z_all, is_feature = True)
    pred = scores.data.cpu().numpy().argmax(axis = 1)
    y = np.repeat(range( n_way ), n_query )
    acc = np.mean(pred == y)*100 
    return acc,losss,lossq

def eval_loss(net,args,rrr,base_loader = None):
    n_way=args.n_way
    k_shot=args.k_shot
    ...
    if alg in ['maml', 'maml_approx']: #maml do not support testing with feature
        ...
        base_loader     = base_datamgr.get_data_loader( loadfile , aug = False )
        net.train()
        losss_mean = net.train_loop(base_loader)
        lossq_mean = 0
        acc_mean = 0
        ...
        # novel_file = os.path.join( feature_file, split +".hdf5")
        
        # cl_data_file = feat_loader.init_loader(novel_file)
        # acc_all = []
        # lossq_all = []
        # for i in range(test_task):
        #     acc,losss,lossq = feature_evaluation(cl_data_file, net, n_query = nquery, **few_shot_params)
        #     acc_all.append(acc)
        #     lossq_all.append(lossq.item())
