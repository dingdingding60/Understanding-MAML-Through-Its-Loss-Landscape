...
from META.data.datamgr import SimpleDataManager
...
def name_surface_file(args, dir_file):
    ...
    surf_file = dir_file[:-3]+"_"+str(args.task_update)+"update"
    if args.losstype =="support":
        surf_file += "_"+str(args.losstype)+"loss"
    if args.task !=0:
        surf_file +="_task"+str(args.task)
    # resolution
...
# def crunch(surf_file, net, dataloader, loss_key, acc_key, comm, rank, args):
def crunch(surf_file, net, w, s, d, loss_key, acc_key, comm, rank, args):
    """
    ...
    weights1_flat = np.concatenate([np.array(w).flatten() for w in d[0]])
    norm1 = np.linalg.norm(weights1_flat)
    ...
    for count, ind in enumerate(inds):
        ...
        lossq,losss, acc = evaluation.eval_loss(net,args,rrr)
        loss_compute_time = time.time() - loss_start
        if args.losstype == "query":
            loss= lossq
        elif args.losstype == "support":
            loss= losss
...
if __name__ == '__main__':
    ...
    # data parameters
    parser.add_argument('--dataset', default='miniImagenet', help='miniImagenet|Omniglot|cifar100')
    ...
    parser.add_argument('--x', default='-1.2:1.2:51', help='A string with format xmin:x_max:xnum')
    parser.add_argument('--y', default='-1.2:1.2:51', help='A string with format ymin:ymax:ynum')
    ...
    parser.add_argument('--vlevel', default=0.2, type=float, help='plot contours every vlevel')
    ...
    parser.add_argument('--test_task', default=4, type=int, help='number of test tasks')
    ...
    #--------------------------------------------------------------------------
    crunch(surf_file, net, w, s, d, 'train_loss', 'train_acc', comm, rank, args)
